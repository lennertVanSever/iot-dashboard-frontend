import {
  loadSensorData,
  groupDataBySensor,
  updateGroupedDataWithNewRecord,
} from "./data.js";
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  setupRealtimeUpdates((newRecord) => {
    ...
    if (sensorId === selectedSensorId) {
      ...
      if (newRecord.data.temperature !== undefined) {
        updateChart(
          "Temperature",
          { timestamp: newRecord.timestamp, value: newRecord.data.temperature },
          sensorId
        );
      }
      if (newRecord.data.pressure !== undefined) {
        updateChart(
          "Pressure",
          { timestamp: newRecord.timestamp, value: newRecord.data.pressure },
          sensorId
        );
      }
      if (newRecord.data.battery_voltage !== undefined) {
        updateChart(
          "Battery Voltage",
          {
            ...
            value: newRecord.data.battery_voltage,
          },
          sensorId
        );
      }
    ...
    const menuItem = document.querySelector(
      `.sensor-item[data-sensor-id="${sensorId}"]`
    );
  ...
  document
    .getElementById("chartTypeSwitch")
